...
const DoctorCard = ({ doc }) => (
  <div className="bg-white rounded-2xl overflow-hidden border border-[#F3F4F6] hover:border-[#fce7f0] hover:shadow-md transition-all flex flex-col group">
    <div
      className="w-full flex-shrink-0 relative overflow-hidden"
      style={{ aspectRatio: '3/4', maxHeight: 195, background: doc.fallbackBg }}
    >
      <img
        src={doc.imageUrl}
        alt={doc.name}
        ...
        onError={(e) => {
          ...
          fb.style.cssText = `width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:${doc.fallbackBg};position:absolute;inset:0`;
          fb.innerHTML = `<span style="font-family:Poppins,sans-serif;font-weight:700;font-size:36px;color:#EC6BA5;opacity:0.5">${doc.initials}</span>`;
    ...
    <div className="p-3 flex flex-col flex-1">
      ...
      <p className="font-inter text-[10px] text-[#9CA3AF] mb-3">{doc.specialization}</p>
      <div className="mt-auto flex flex-col items-center gap-1">
        ...
        <span className="font-inter text-[10px] text-slate-500 font-medium">{doc.hours}</span>
      </div>
    </div>
  </div>
);
...
/* ══════════════════════════════════════
   PHONE CHAT MOCKUP — compact & clean
══════════════════════════════════════ */
const PhoneMockup = () => (
  <div style={{
    ...
    <div style={{ flex: 1, background: 'white', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, background: '#f8f9fa', padding: '8px 6px', display: 'flex', flexDirection: 'column', gap: 6, overflow: 'hidden' }}>
        ...
        <div style={{ marginLeft: 'auto', maxWidth: '80%', background: 'white', borderRadius: '10px 10px 2px 10px', padding: '4px 6px', boxShadow: '0 1px 4px rgba(0,0,0,0.07)', border: '1px solid #f1f5f9' }}>
          <p style={{ fontSize: 5, color: '#475569', lineHeight: 1.55, margin: 0 }}>
            Halo dok, saya ingin konsultasi tentang jerawat.
          </p>
        ...
        <div style={{ marginRight: 'auto', maxWidth: '80%', background: '#FFF0F6', borderRadius: '10px 10px 10px 2px', padding: '4px 6px', boxShadow: '0 1px 4px rgba(236,107,165,0.1)' }}>
          <p style={{ fontSize: 5, color: '#1e293b', lineHeight: 1.55, margin: 0 }}>
            Halo! Silakan kirim foto area kulit yang ingin dikonsultasikan ya 😊
          </p>
          <p style={{ fontSize: 4, color: '#EC6BA5', margin: '2px 0 0' }}>09:42</p>
        </div>

        {/* Foto placeholder → kanan */}
        <div style={{ marginLeft: 'auto', width: '48%', height: 30, background: '#fce7f0', borderRadius: 8, border: '1px solid #fce7f0', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3 }}>
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#EC6BA5" strokeWidth="2">
            <rect x="3" y="3" width="18" height="18" rx="2"/>
            <circle cx="8.5" cy="8.5" r="1.5"/>
            <polyline points="21,15 16,10 5,21"/>
          </svg>
          <span style={{ fontSize: 4, color: '#EC6BA5', fontWeight: 600 }}>foto.jpg</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 items-start">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="flex flex-col">
            ...
            {/*
              Card redesign:
              - Background gradient ungu muda di sisi foto
              - Foto apoteker full height di kiri
              - Checklist lebih besar dan spasi proporsional
              - Tombol dengan shadow yang lebih kuat
            */}
            ...
            >
              {/* Kiri: foto dengan overlay gradient */}
              <div className="relative flex-shrink-0 overflow-hidden" style={{ width: '42%', background: 'linear-gradient(160deg, #f3e8ff 0%, #ede9fe 100%)' }}>
                <img
                  ...
                  onError={(e) => { e.target.src = 'https://randomuser.me/api/portraits/women/55.jpg'; }}
                />
                ...
                <ul className="flex flex-col gap-2 flex-1 mb-4">
                  {apotekerItems.map(({ label }) => (
                    <li key={label} className="flex items-center gap-2">
                      ...
                      <span className="font-inter text-[12px] font-medium text-slate-700 leading-snug">
                        {label}
                      </span>
                    </li>
                  ))}
          ...
          <div className="flex flex-col">
            <div className="flex items-center mb-4 h-[28px]">
              {/* Label dengan dot accent */}
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-[#EC6BA5]" style={{ boxShadow: '0 0 6px rgba(236,107,165,0.6)' }} />
                <span className="font-poppins font-bold text-[11px] text-[#EC6BA5] tracking-[0.12em] uppercase">
                  Konsultasi Online
                </span>
              </div>
            </div>
            ...
            {/*
              Card redesign:
              - Background gradient pink yang lebih rich
              - Teks lebih besar dan spacing lebih lapang
              - Phone mockup proporsional di kanan
            ...
            */}
            ...
            >
              ...
              <div className="absolute pointer-events-none" style={{ width: 60, height: 60, borderRadius: '50%', background: 'rgba(236,107,165,0.06)', bottom: 20, left: '30%' }} />

              {/* Konten kiri */}
              <div className="flex flex-col justify-between relative z-10" style={{ flex: 1, padding: '16px 12px 14px 16px', minWidth: 0 }}>
                <div>
                  {/* Heading */}
                  <h3 className="font-poppins font-bold text-slate-800 leading-snug mb-4"
                    style={{ fontSize: 13.5 }}>
                  ...
                  <ul className="flex flex-col gap-2">
                    {onlineItems.map((item) => (
                      <li key={item} className="flex items-center gap-2">
                        <div className="w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0"
                          style={{ background: 'rgba(236,107,165,0.15)' }}>
                ...
                <a
                  ...
                  style={{
                    ...
                    boxShadow: '0 4px 14px rgba(236,107,165,0.38)',
                  }}
                >
              ...
              {/* Phone mockup kanan — sedikit naik dari bawah */}
              <div
                className="relative z-10 flex-shrink-0 flex items-end justify-center"
                style={{ width: 120, paddingRight: 8, paddingTop: 8 }}
              >
                <div
